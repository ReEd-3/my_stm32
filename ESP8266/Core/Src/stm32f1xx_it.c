...
extern uint8_t tx_buffer[BUFFER_LEN]; //发送缓冲区
extern uint8_t rx_buffer[BUFFER_LEN]; //接收缓冲区
  ...
  /* USER CODE BEGIN USART1_IRQn 0 */
	if (__HAL_UART_GET_FLAG(&huart1, UART_FLAG_IDLE) != RESET){ //如果发生空闲中断
	  ...
	  __HAL_UART_CLEAR_IDLEFLAG(&huart1);  //清除空闲标志
		...
		head = head + datalen;
		if(head >= BUFFER_LEN){ // 处理头部溢出
			head = head - BUFFER_LEN;
		}
		if(all_datalen > head){ // 处理尾部溢出
			datalen = all_datalen - head; //计算数据长度
		}
		else{
			datalen = all_datalen + BUFFER_LEN - head;
		}
		...
		for(int i = head; i < head + datalen; i++){ // 将rx数据写入tx
			if(i < BUFFER_LEN){
				...
				tx_buffer[i - head] = rx_buffer[i - BUFFER_LEN];
			}
		}
