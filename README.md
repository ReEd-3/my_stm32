# my_stm32

## Tips

- 2025.11.04  
    1. keil无法对**国产芯片**识别，可以连接但不能烧录。  
    2. stm32f1c103c8t6默认LED连接PC13(或PB12)。  

- 2025.11.05  
    1. 保证电路处处通路，尤其是排针与系统板接触。  
    2. 四脚按键同侧两脚相连  

- 2025.11.06  
    1. PWM含有三个重要参数  
        - 频率  
        - 周期  
        - 占空比  
    2. PWM可以通过控制**高低电平占比**来控制LED亮度  
    3. 呼吸灯的流畅程度和arr有很大关系  
    4. ccr的数值决定了PWM的占空比  
    5. **duty**(占空比)和arr,ccr之间满足**ccr = duty * (arr+1)**  

- 2025.11.07
    1. 舵机旋转的流畅程度也和arr有很大关系
    2. PWM信号频率 = 时钟频率 ÷ psc ÷ arr (这表明了arr越PWM信号频率越小输出越不流畅)  

- 2025.11.08
    1. 按键控制中GPIO_INPUT需要设置为**上拉(Pull up)**
    2. 编写大型工程时将**功能模块化**很重要，边编写边测试，可以利用默认LED指示灯用于测试
    3. 为程序设置**状态机**可以有效实现对复杂情况的处理
    4. 对双击操作进行消抖时要保证第二次点击充分消抖防止双击后单击

- 2025.11.09
    1. stm32系统板上的**TX，RX**分别和USB转TTL上的**RXD，TXD**连接
    2. 非阻塞中使用**HAL\_TIM\_PeriodElapsedCallback(TIM\_HandleTypeDef \*htim)**
    和**HAL\_UART\_TxCpltCallback(UART\_HandleTypeDef \*huart)**进行回调函数调用（计时器arr每溢出一次进行一次回调）
    3. 阻塞编程中需要设置**timeout**防止程序被一直截断

- 2025.11.10
    1. **空闲中断(IDLE)**不会自动清除，需要手动清除，不清除会导致**反复进入空闲中断**
    2. 进入空闲中断使用
        ```c
        void USART1_IRQHandler(void)
        {
            if (__HAL_UART_GET_FLAG(&huart1, UART_FLAG_IDLE) != RESET){//空闲中断
                /* 代码部分 */
            }
        HAL_UART_IRQHandler(&huart1);
        }
        ```
        这段代码原型放在**stm32f1xx_it.c**中
    3. 不要使用`memset`格式化字符串，会出现无法输出问题
    4. 在空闲中断中使用`HAL_UART_AbortReceive_IT(&huart1);`中断接收，否则会导致第二次入时**首个字符丢失**的情况

- 2025.11.13
    1. 在DMA中接收数据时，DMA指向一个缓冲区(一个uint8_t数组)，在缓冲区中进行数据的写入，缓冲区将要溢出时会从缓冲区头部进行写入
        例如
        ```text
        
        最开始          输入“114514”            输入“0721”
        DMA                  DMA                 DMA
        |                    |                   |
        V                    V                   V
        --------        114514--                21451407
        ```
    2. 可以使用`HAL_GPIO_TogglePin(GPIOC,GPIO_PIN_13)`翻转板载LED灯的电平进行调试(例如观察是否进入FreeRTOS，是否发生了中断，是否进入了循环)
    3. FreeRTOS中的不同线程有优先级之分，需要确保并行的线程优先级相同
    4. 空闲中断中使用`__HAL_DMA_DISABLE(huart1.hdmarx)`停用DMA后，再次打开DMA需要用`__HAL_DMA_ENABLE(huart1.hdmarx)`, 
    如果使用`HAL_UART_Receive_DMA(&huart1, rx_buffer, sizeof(rx_buffer))`直接再次启用DMA会导致后续传输的数据出错(传输新的数据返回旧的数据，传输多次才能返回新的的数据)
    5. FreeRTOS可以在**freertos.c**中建立多个线程，在调用`osKernelStart()`之后程序会忽视main中的内容，执行**freertos.c**中的线程 
    freertos和SYS时钟源需要配置不同的时钟，例如一个使用系统时钟另一个使用TIM1， 
    freertos中进行阻塞需要使用`osDelay()`

- 2025.11.14
    1. MPU6050模块的**AD0引脚**需要连接**GND引脚**
    2. MPU6050上电后通常处于**睡眠模式**，需要**配置寄存器**使其进入工作状态 
    使用`HAL_I2C_Mem_Write`函数操作MPU6050的内部寄存器
    例如 
    ```text

	uint8_t SendAddress = 0x6b; 
	uint8_t SendData = 0x00; 
	HAL_I2C_Mem_Write(&hi2c1,0xD1,SendAddress,1,&SendData,1,0xff); //向电源管理寄存器中写入0x00

    ```
    3. MPU6050初始化总共需要需要配置
        - 唤醒设备
        - 设置采样率分频
        - 设置低通滤波器
        - 设置陀螺仪量程
        - 设置加速度计量程

- 2025.11.15
    1. **卡尔曼滤波** 
        1. 卡尔曼滤波分为两步，第一步为先验阶段，第二步为后验阶段
        2. 先验阶段需要完成两个计算 
            - x_k = F \* xk + B \* uk 
            新的预测状态**x_k**等于上一时刻的最佳估计**xk**乘以状态转移矩阵**F**，再加上控制输入**B\*uk**的影响
            - P_k = F \* Pk \* FT + Q
            新的预测不确定性**P_k**等于上一时刻的不确定性**Pk**通过状态转移矩阵**F**进行变换，再加上过程噪声**Q**
        3. 后验过程需要完成三个计算 
            - Kk = P_K \* HT / (H \* P_k \* HT + R) 
            分母**H \* P_k \* HT + R**是**预测的测量值**的不确定性**H \* P_k \* HT**加上“实际测量值”的不确定性**R** 
            如果测量噪声**R**很小（传感器很精确），分母趋近于**H \* P_k \* HT**，卡尔曼增益**Kk**会变大，将更相信测量值 
            如果预测不确定性**P_**很小（模型非常可靠），分子**P_K \* HT**变小，卡尔曼增益**Kk**会变小，将更相信预测值 
            - xk = x_k + Kk \* (zk - H \* x_k) 
            **(zk - H \* x_k)**被称为 “新息” 或 “残差” 。它是实际测量值与预测的测量值之间的差异 
            将这个差异乘以卡尔曼增益**Kk**，然后用它来修正我们的先验预测**x_k**
        4. 最终输出**xk**作为最终结果

- 2025.11.17
    1. 计算**欧拉角**
        1. **Pitch**角（俯仰角） & **Roll**角（偏航角）
            - 这两个角度均为绕某**水平**轴旋转的角度，可以使用**积分**计算法（对某个轴上的角速度进行积分） 
            也可以使用重力加速度的分量进行反三角函数计算
            - 这是因为大部分情况下重力加速度在z轴有较大的分量，可以进行相对精确的计算，将**加速度（Acc）**的数据和**陀螺仪（Gyr）**的数据进行**互补滤波**， 
            可以得到较稳定的数据
            - **互补滤波** 
            Acc得到的角度（以下简称Acc）和Gyr（以下简称Gyr）得到的角度进行互补滤波计算最终数据（以下简称Fin）时会遵守 
            > **Fin = α \* Acc + (1 - α) \* Gyr** 
            > 
            陀螺仪会产生零偏，变化**缓慢**，属于**低频**误差，加速度计会受到噪声干扰，变化**快速**，属于**高频**噪声， 
            因此要滤掉**Gyr低频**和**Acc高频**， 
            其中高低频的分界线称为**转折频率（f_c）**，对数据进行采样的频率叫做**采样频率（f_s）** 
            则α满足 
            > **α = 1 / (1 + (f_c / f_s))** 
        2. 计算**Yaw**角（偏航角）
            - 由于重力在水平面上的分量很小，因此计算Yaw角时我采用了**忽略微小变化量**的算法（忽略角速度在0.4以下的变化量）
    2. **VOFA+**中在**FireWater**模式下会对传输的数据进行捕获，传输格式需要是(例如)
        > %d,%d,%d,%d\n
        > 或者
        > data:%d,%d,%d,%d,%d\n (捕获冒号后的数据)
    3. 在**VOFA+**中，如果在转动模型欧拉角参数直接输入计算出来的欧拉角模型的运动姿态会和实际物体的运动成镜像，如果要得到实际的运动姿态， 
    Roll角参数需要输入**0** **-** **Roll**(Roll为直接计算出的Roll角)

- 2025.11.18
    1. 使用OLED需要提前编写OLED驱动文件(OLED.c,OLED.h)
    2. USB转TTL直接连接电脑可以在设备管理器中检测到，但是连接ESP8266后无法检测到USB连接任何设备， 
    有可能是因为USB转TTL不能直接对ESP8266进行供电，需要使用外部电源（我这里是使用了MP1584，正极输入用USB转TTL的5V引脚，负极输入用USB转TTL的GND引脚）

## Files

### 1.基础外设掌控（硬件交互）

1. LED001
    - 实验性工程，PB8以1Hz实现高低电平翻转  

2. LED002
    - 实验性工程，PC13和PB12以1Hz实现高低电平翻转  

3. LED003
    - 开关连接PB8控制LED的点亮和熄灭，使开关按下一次LED转换一次状态，  
    PC13控制默认LED的状态使开关按下时默认LED熄灭，其余时刻点亮  

4. Breath_LED  
    - 呼吸灯，使LED亮度随着时间成正弦变化，代码块中使用for循环和delay使时间i递增,  
    用sin函数计算渐亮时的ccr，cos函数计算渐暗时的ccr  

5. Servo_control
    - 舵机控制，通过**PWM**控制舵机旋转到0°，90°，180°，间隔1s  
    - 还可以通过**for循环**控制舵机的转动速度（见while循环中注释掉的代码）

6. SERVO_KEY  
    - 按键实现舵机特定角度的控制，为按键状态和事件状态设置了多种状态
    - 执行过程可分为
        1. 检测**按键状态**
        2. 根据**按键状态**设置不同的**按键事件**
        3. 根据**按键事件**进行**舵机位置**的控制
        4. 根据舵机位置输出相应的**PWM**

### 2.串口通信与优化（人机交互）

1. USART
    - 简单的串口通信，程序执行时使用`HAL_UART_Transmit_IT(&huart1, 字符串名称, 字符串长度，timeout)`阻塞的方法发送`"hello world"`

2. Time_Transmit
    - 根据时钟进行时间数据的传输，每**1/10秒**传输一次系统时间，
    - 执行过程分为
        1. **时钟2**进行回调时，**系统时间**+1，**时钟状态**变为可用
        2. **USART1**进行回调时，**串口状态变**为可用
        3. 如果**时钟状态**和**串口状态**均可用，则计算出时分秒的数值，**格式化字符**串生成时间，并发送

3. LED_Command
    - 上位机使用**VOFA+**发送命令"ON","OFF",使PC13连接的LED相应开关并相应返回"LED_ON","LED_OFF"
    - 执行过程分为
        1. 打开**接收回调**，将输入的数据保存在`rx_buffer`中
        2. 对比`rx_buffer`中的数据和命令，如果输入的是命令则执行相应的代码
    - 要注意在读取到`\n`时在`rx_buffer`末尾加`\0`

4. Idle_Interruption
    - 使**用接收中断**和**空闲中断**，将输入的值原封不动返回
    - 执行过程分为
        1. 打开**接收中断回调**和**空闲中断回调**
        2. 接收到数据时，将每个字节存放在缓冲区(这将触发若干次接收中断)，最后触发一次空闲中断回调，结束传输
        3. 在主循环中向上位机发送刚刚传输的数据

5. DMA_IO
    - 使**DMA接收数据**和**空闲中断**，将输入的值原封不动返回
    - 执行过程与**4**类似，但是不需要对接收中断进行处理，接收数据由**DMA**完成
    - 使用注意事项见**2025.11.13**

6. ESP8266
    - **ESP8266模块**连接**stm32**开发板，开发板通过**USB转TTL**和电脑相连，在**VOFA+**中查看串口返回的信息
    - 首先在stm32中写入**DMA_IO**项目中的代码，使得STM32可以将**USART1**中的信息原封不动地从**USART2**中发送到上位机
    - 初始化时设置ESP8266模式为**station**,并且打开**多连接**
    - 再连接WIFI，创建**TPC服务器**，端口号8080
    - 手机用网络调试工具连接TPC服务器，用ASCII编码写入信息
    - stm32收到信息后判断是否有`IPD+`字样，并且找到`:`字样，提取`:`后边的字符串，判断是否有`LED_ON`或者`LED_OFF`
    并且进行相应的反应
    - 流程图可以表示为
    ```text

    初始化
           ②->"OK"等响应信息        ③->USART1中收到的信息          
    ESP8266<=====USART1=====>stm32<=====USART2=====>USB转TTL
           ①<-初始化AT指令

    工作状态
        ①->发送指令                  ②->发送收到的数据
    手机<======TPC服务器=====>ESP8266<=====USART1=====>stm32
                                                    ③根据收到的数据做出响应
    ```

### 3.引入实时操作系统（系统整合）

1. FreeRTOS
    - 使用**STM32CUBEMX**开启FreeRTOS，保证两个并行线程之间优先级相等
    - 编写**呼吸灯**程序和**DMA接收并处理数据**程序

### 4.传感器与数据融合（综合应用）

1. MPU6050
    - 连接并**初始化**MPU6050模块，读取**三维加速度**和**三维角速度**
    - 通过串口打印相应的数据
2. Kalman_Filter
    - 同4.1获取数据，显示这些数据的**波形图**
    - 通过**卡尔曼滤波**的方法获取更平滑的曲线
    - 通过获取的数据解算出欧拉角，在VOFA+中可以显示出物体的真实姿态
    - 通过在OLED驱动，将数据打印在OLED屏幕上

